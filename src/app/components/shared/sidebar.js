  ...
  return (
    <div className={'lw-sidebar-container'} style={{width: SIDEBAR_WIDTH, minWidth: SIDEBAR_WIDTH, overflowY: 'hidden', flexWrap: 'nowrap', maxHeight: '100%'}}>
      ...
      <div className={`lw-sidebar-settings-panel ${showSettings ? '' : 'hidden'}`}>
        <SidebarButton onClick={() => setShowSettings(false)} style={{color: '#a4afb7'}}><img alt={Localize.text('Dashboard icon', 'sidebar')} src={`${publicPath}/images/icons/icon-back.png`} /> <Localize context={'universal'}>Back</Localize></SidebarButton>
      ...
      <SidebarButton active={activeView === activeViews.DASHBOARD} onClick={onDashboard}><img alt={Localize.text('Dashboard icon', 'sidebar')} src={`${publicPath}/images/icons/icon-home.png`} /> <Localize context={'sidebar'}>Dashboard</Localize></SidebarButton>
      <SidebarButton active={activeView === activeViews.PORTFOLIO} onClick={onPortfolio}><img alt={Localize.text('Portfolio icon', 'sidebar')} src={`${publicPath}/images/icons/icon-wallet.png`} /> <Localize context={'sidebar'}>Portfolio</Localize></SidebarButton>
      <SidebarButton active={activeView === activeViews.TRANSACTIONS} onClick={onTransactions}><img alt={Localize.text('Dashboard icon', 'sidebar')} src={`${publicPath}/images/icons/icon-history.png`} /> <Localize context={'sidebar'}>Transactions</Localize></SidebarButton>
      <SidebarButton onClick={() => setShowSettings(true)}><img alt={Localize.text('Settings icon', 'sidebar')} src={`${publicPath}/images/icons/icon-settings.png`} /> <Localize context={'sidebar'}>Settings</Localize></SidebarButton>
      <SidebarButton onClick={onLockClick}><img alt={Localize.text('Lock icon', 'sidebar')} src={`${publicPath}/images/icons/icon-lock-closed.png`} /> <Localize context={'sidebar'}>Lock Wallet</Localize></SidebarButton>
      <SidebarDivider />
